...
export default function FocusInputLottie() {
  ...
  const handleUserNameFocus = () => {switchState("Following")};
  const handlePasswordFocus = () => {switchState(showPassword ? "Peeking" : "Covering")};
  const handleBlur = () => {switchState("Blinking")} 

  useEffect(()=>{
    animationRef.current?.interactivity?.inputs.set('name_length', Math.min(userName.length / 32, 1))
  }, [userName])
      ...
      <div className='details'>
        ...
        <div className='detail-label'>
          ...
          <label className="show-password">
            <input type="checkbox" checked={showPassword} onChange={() => setShowPassword(!showPassword)} /> Show
          </label>
        </div>
